...
void main(){
	...
	float specBase = dot(reflect(normalize(cameraPos), texture( NormalTex, vec2(UV.x,-UV.y)).rgb),cameraPos);
	specBase = clamp(specBase,0,1);
	vec3 fp = fract(0.7 * Position_worldspace + 9 * noise3( Position_worldspace * 0.04).r + 0.1 * cameraPos);
	fp *= (1 - fp);
	...
	float LightPower = 0.5;//1.0;//40.0;
	
	vec3 MaterialDiffuseColor = texture( DiffuseTex, UV ).rgb *2;
	...
	vec3 colorFin = glitter * sparkle *1.5+ // *0.5+
		 MaterialAmbientColor *5 /**glitter *sparkle*/+
		 MaterialDiffuseColor*2 * LightColor *LightPower* cosTheta / (distance*distance);// +
